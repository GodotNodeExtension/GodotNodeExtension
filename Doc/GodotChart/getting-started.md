...
`ChartKind` covers all 24 built-in kinds (`Bar`, `Line`, `Area`, `Scatter`, `RangeArea`, `Pie`, `Donut`,
`Radar`, `Violin`, `Box`, `Candlestick`, `Heatmap`, `Treemap`, `Sunburst`, `Sankey`, `Chord`,
`Gauge`, `Funnel`, `Waffle`, `Timeline`, `Lollipop`, `Milestone`, `GeoArea`, `GeoBubble`) - 24 kinds backed by 22 `Mark`
...
- [Chart Types](chart-types.md) — Learn how to build all 24 chart kinds
