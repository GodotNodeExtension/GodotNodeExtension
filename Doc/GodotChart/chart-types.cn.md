...
**气泡自带坐标。** `GeoBubbleMark` 为每一行在它携带的坐标上画一个气泡 —— 位置通道装的是经度与纬度（`X` 与 `Y`，
线性标度不强制零基线），`Size` 决定半径、`Color` 决定填充，与散点类 mark 在坐标轴上的做法完全一样。坐标缺失或不是
数字的行会被跳过，其余行继续绘制。`ChartView` 的 `GeoBubble` 类型会自动为这些行取景，所以一张只有经纬度的表不需要
页面自己去设视图就能显示它的点。
