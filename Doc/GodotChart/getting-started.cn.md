...
`ChartKind` 覆盖全部 25 种内置类型（`Bar`、`Line`、`Area`、`Scatter`、`RangeArea`、`Pie`、`Donut`、
`Radar`、`Violin`、`Box`、`Candlestick`、`Heatmap`、`Treemap`、`Sunburst`、`Sankey`、`Chord`、`Gauge`、
`Funnel`、`Waffle`、`Timeline`、`Lollipop`、`Milestone`、`GeoArea`、`GeoBubble`、`GeoFlow`）——25 种类型由 23 个 `Mark` 类实现
...
- [图表类型](chart-types.cn.md) — 了解全部 25 种图表类型的构造方法
