...
GodotChart 是一个基于 **Grammar of Graphics** 设计理念的 Godot 声明式图表库，使用 C# 和 SkiaSharp 构建。它提供流畅的 API 来创建丰富的交互式图表：25 个 `ChartKind`（由 23 个 `Mark` 类实现，另有注解 mark `SectionMark`）、动画系统、主题定制及完整的用户交互体验。
...
| [图表类型](chart-types.cn.md) | 25 种图表类型的详细说明与示例代码 |
...
| 场景 | 展示内容 |
|---|---|
| `BasicsDemo.tscn` | 库里的全部图表类型（25 种；柱状与面积另有分组/堆叠变体格），以及 `SectionMark` 参考线 |
